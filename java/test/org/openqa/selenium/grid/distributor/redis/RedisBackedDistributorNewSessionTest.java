...
class RedisBackedDistributorNewSessionTest {
  ...
  @Test
  void shouldRejectEmptyCapabilities() {
    ...
    Either<SessionNotCreatedException, org.openqa.selenium.grid.data.CreateSessionResponse> result = 
        distributor.newSession(request);

    assertThat(result.isLeft()).isTrue();
    assertThat(result.left().getMessage()).contains("No capabilities found");
  }

  @Test
  void shouldFailWhenNoNodesAvailable() {
    ...
    assertThat(result.isLeft()).isTrue();
    assertThat(result.left().getMessage()).contains("Unable to create session");
  }
