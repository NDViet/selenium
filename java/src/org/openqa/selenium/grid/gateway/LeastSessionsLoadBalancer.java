...
/**
 * Least Sessions load balancer that routes to the instance with fewest active sessions.
 ...
 */
public class LeastSessionsLoadBalancer implements LoadBalancingStrategy {

  private final CapabilityMatcher capabilityMatcher;
  private final RoutingCache routingCache;

  public LeastSessionsLoadBalancer() {
    this.capabilityMatcher = new CapabilityMatcher();
    this.routingCache = new RoutingCache();
  }

  public LeastSessionsLoadBalancer(CapabilityMatcher capabilityMatcher) {
    this.capabilityMatcher = capabilityMatcher;
    this.routingCache = new RoutingCache();
  }

  public LeastSessionsLoadBalancer(CapabilityMatcher capabilityMatcher, RoutingCache routingCache) {
    this.capabilityMatcher = capabilityMatcher;
  ...
  public Optional<GridInstance> selectGridInstance(
    ...
    Optional<GridInstance> selected =
        finalInstances.stream().min(Comparator.comparingInt(GridInstance::getSessionCount));

    // Mark as pending if found
    selected.ifPresent(instance -> routingCache.markPending(instance.getId()));
